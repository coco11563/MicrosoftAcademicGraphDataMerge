...
object combine {
  def main(args: Array[String]): Unit = {
    ...
    val t1 = s"hadoop fs -rmr $out_path" !
    val t2 = s"hadoop fs -rmr $paper_temp_out_path_v2" !
    ...
    sparkSession.read.table("oadv2.merged_table").write.mode(SaveMode.Overwrite).json(paper_temp_out_path_v2)
    ...
    val v1_kv = v1_paper_rdd.map(j => (Option(j.values("year")).getOrElse(0).##.toString + j.values("title").##.toString, (j, 1)))
    val v2_kv = v2_paper_rdd.map(j => (Option(j.values("year")).getOrElse(0).##.toString + j.values("title").##.toString, (j, 2)))
    val final_json = (v1_kv union v2_kv)
        ...
        }).values.filter(r => r._2 == 2).map( f => f._1 ).map(compact(_))
    final_json.saveAsTextFile(out_path)
    sparkSession.read.json(out_path).write.mode(SaveMode.Overwrite).saveAsTable("oadv2.v1_v2_merged_table")
